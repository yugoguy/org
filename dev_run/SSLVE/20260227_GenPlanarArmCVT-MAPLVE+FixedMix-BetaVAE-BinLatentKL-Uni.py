#@title PlanarArmCVT MAP SSLVE BinKL FixedMix (Generalized)
import numpy as np
import random
import torch
...
GAMMA_BIN_KL = 1e-3  #@param {type:"number"}
...
fitness_fn = lambda info: np.mean(info['angle_variance']) if isinstance(info['angle_variance'], list) else info['angle_variance']
