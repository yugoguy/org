#@title GenPlanarArmCVT SSLVE BoltzmannMix Flow — Disconnected (Sin/Cos Pie)
import numpy as np
import random
import torch
...
FITNESS = "sincos_pie"  #@param ["angle_variance", "sine_dependency", "sincos_pie"] {type:"string"}
N_PIES = 4  #@param {type:"integer"}
...
def _sincos_pie_fn(info):
    x, y = info['end_effector']
    slice_id = int((np.arctan2(y, x) + np.pi) / (2 * np.pi / N_PIES)) % N_PIES
    if slice_id % 2 == 0:
        return _get_metric(info, 'sine_dependency')
    else:
        return _get_metric(info, 'cosine_dependency')

FITNESS_FNS = {
    'angle_variance': lambda info: _get_metric(info, 'angle_variance'),
    'sine_dependency': lambda info: _get_metric(info, 'sine_dependency'),
    'sincos_pie': _sincos_pie_fn,
}
...
print(f"Fitness: {FITNESS}" + (f" (N_PIES={N_PIES})" if FITNESS == "sincos_pie" else ""))
