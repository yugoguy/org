...
def record_stats(pop):
    raw_fits = np.array([problem.fitness(binarize(ind)) for ind in pop])
    cvs = np.array([problem.constraint_violation(binarize(ind)) for ind in pop])
...
print(f"Constraint violation: {problem.constraint_violation(best_binary):.4f}")
print(f"Ratio of 1s: {sum(best_binary)/N:.2f}")
print(f"Feasible: {problem.is_feasible(best_binary)}")
