...
globals [
  ...
  carbon-credit-price  ; will be updated each round via random walk
  ...
  effective-token-supply ; update ... amount of tokens owned by non-exchange agents.
...
users-own [
  ...
  risk-aversion ; 固定：リスク回避度（agentごとに異なる）
...
to go ; out
  ...
  foreach range intra-period-trading-rounds [
    ...
    set effective-token-supply sum [token-holdings] of users with [agent-type != "exchange"]
    ...
    let rounds-per-year (12 * intra-period-trading-rounds)
    let dt  (1.0 / rounds-per-year)            ; fraction of a year per round
    let drift-per-step (carbon-drift)          ; annual drift
    let vol-per-step   (carbon-vol)            ; annual volatility
    set carbon-credit-price carbon-credit-price *
    exp((drift-per-step - 0.5 * (vol-per-step ^ 2)) * dt +
      vol-per-step * sqrt(dt) * (random-normal 0 1))
...
to initialize-variables ; out
  ...
  set effective-token-supply initial-token-supply
  ...
  set carbon-credit-price initial-carbon-credit-price
...
to define-agents ; in
  ...
  set risk-aversion 0.1 + random-float 9.9 ; 一様分布
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;------------write computation of optimal token holdings here that you derived from the utility function------------
...
; governance-value ... total governance value
; moving-average-return window ... expected token investment return per token
; last price-record ... latest price of token
; effective-token-supply ... latest supply of tokens that are owned by the platform users, not by exchange (if held by exchange, it means its not sold)
; risk-free-rate ... risk free rate
; (last price-record) * amount ... value of the current token holdings
; (governance-value) * amount * voting / effective-token-supply ... governance value of the holdings
; (moving-average-return window) * (last price-record) * amount ... expected asset investment return
; ((moving-average-return window) - risk-free-rate) * (last price-record) * amount ... expected asset investment risk premium
...
; WRITE ME13
to-report utility-function [amount]
  let G governance-value * voting / effective-token-supply
  let rv (moving-average-return window - risk-free-rate)
  let S (1 + rv) * last price-record
  let gamma risk-aversion

  let carbon carbon-credit-price

  let benefit (G + S + carbon) * amount
  let cost gamma * amount * amount
...
to-report optimal-token-holdings
  let G governance-value * voting / effective-token-supply
  let rv (moving-average-return window - risk-free-rate)
  let S (1 + rv) * last price-record
  let gamma risk-aversion
  ...
  let a-star (G + S + carbon) / (2 * gamma)
  report max list 0 a-star
end
...
to-report moving-average-variance [window_length] ; either
  ifelse (length return-record) < 2 [
    report 1
  ][
    let record_length (length return-record)
    set window_length min list record_length window_length
    report variance ( sublist return-record (record_length - window_length) record_length)
  ]
end
...
to-report scheduled-token-issue-amount ; out
  ...
  ifelse token-supply < supply-cap [
    ifelse ticks < supply-inflection-ticks [
      set additional-amount ((pre-token-issue-rate * (2 ^ (ticks / pre-supply_doubling_tick_length))))
      report min list additional-amount (supply-cap - token-supply)
    ][
      set additional-amount ((post-token-issue-rate * (0.5 ^ (ticks / post-supply_halving_tick_length))))
      report min list additional-amount (supply-cap - token-supply)
    ]
  ][
    report 0
      ...
      if buy-volume > 0 [
        ask x [
          ...
          ifelse agent-type = "exchange" [
            ...
            set trade-value-numerator trade-value-numerator + (bid-price * amount)
          ]
        ]
      ]
    ]
...
to place-orders ; in
  if (random-float 1) < activeness [
    ...
    ifelse wishing-amount > 0 [
      ...
      set bid-price max list 1e-10 per-token-valuation ; pure valuation approach
      ...
      if wishing-amount < 0 [
        if (token-holdings - locked-holdings) > 0 [
          ...
          set ask-price max list 1e-10 per-token-valuation ; pure valuation approach
...
to-report token-utility-valuation ; out
  report (last price-record) * effective-token-supply / ((1 + annual-discount-rate) ^ (ticks / intra-period-trading-rounds / 12)) ; in present value
end
...
to record-price&return ; out
  let vwap (trade-value-numerator / (token-trade-volume * 2))
  ...
  foreach range schedule-horizon [ t ->
    ifelse scheduled-supply-t < supply-cap [
      ifelse t < supply-inflection-ticks [
        set scheduled-supply-t scheduled-supply-t + (min list (supply-cap - scheduled-supply-t) ((pre-token-issue-rate * (2 ^ (t / pre-supply_doubling_tick_length)))))
        plot scheduled-supply-t
      ][
        set scheduled-supply-t scheduled-supply-t + (min list (supply-cap - scheduled-supply-t) ((post-token-issue-rate * (0.5 ^ (t / post-supply_halving_tick_length)))))
        plot scheduled-supply-t
      ]
    ][
      plot scheduled-supply-t
...
PENS
"トークン現在価値" 1.0 0 -2674135 true "" "if ticks > 1 [plot (observer-governance-value + observer-terminal-value + token-utility-valuation) / effective-token-supply]"
...
"保有" 1.0 0 -2674135 true "" "if ticks > 0 [plot effective-token-supply]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "ifelse (return-record != 0) and (token-trade-volume > 0) [\n  if (length return-record) > 0 [\n    plot (last return-record)\n  ]\n][plot 0]"
...
338
トークンあたりのユーティリティ価値
token-utility-valuation / effective-token-supply
17
1
11
...
338
トークンあたりのガバナンス価値
(observer-governance-value + observer-terminal-value)/ effective-token-supply
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 1 [plot last price-record]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plot carbon-credit-price]"
