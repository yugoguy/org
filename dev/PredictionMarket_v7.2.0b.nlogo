...
to-report optimality
  let gap (q-yes - q-no)
  let k-diff-star-abs abs ( ( liquidity * ( ln ( effective-belief / ( loss-aversion * ( 1 - effective-belief ) ) ) ) ) - gap )

  let k-step step-constraint k-diff-star-abs

  let baseline expected-utility-function 0 0
  let yes-long-eutility expected-utility-function k-diff-star-abs 0
  let yes-short-eutility expected-utility-function (- k-diff-star-abs) 0
  let no-long-eutility expected-utility-function 0 k-diff-star-abs
  let no-short-eutility expected-utility-function 0 (- k-diff-star-abs)
  let choice ( list (list k-step 0) (list 0 (- k-step)) (list 0 k-step) (list (- k-step) 0) )
