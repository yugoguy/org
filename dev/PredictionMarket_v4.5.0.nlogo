...
to-report risk-neutral-decision ; in bettor ; box
  let gap (q-yes - q-no)
  let k ((liquidity * ln (effective-belief / (1 - effective-belief))) - gap)
...
to-report network-information-sharing [x]
  ...
  ifelse any? nbrs [
    let avg belief
    ifelse confirmation-bias = true [
      set avg  (sum [ belief * (1 - ( (distance myself) / scale)) * (exp (- abs (belief - my-belief) / confirmation)) ] of nbrs) / (count nbrs)
    ][
      set avg  (sum [ belief * (1 - ( (distance myself) / scale)) ] of nbrs) / (count nbrs)
    ]
