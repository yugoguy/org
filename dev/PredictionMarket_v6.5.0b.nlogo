...
to go
  repeat market-attempts [
    ...
    record
    if ((max list weighted-prediction (1 - weighted-prediction)) > (1 - 1e-4)) and early-stop? [stop]
  ]
end
...
to-report optimality
  let gap (q-yes - q-no)
  let k-diff-star ( liquidity * (ln ( effective-belief / ( 1 - effective-belief ) ) )  ) - gap

  let risk-adjust clip (1 + liquidity * CARA)

  set k-diff-star k-diff-star / risk-adjust

  let k-step step-constraint k-diff-star

;  let baseline utility-function 0 0 "yes"
;  let yes-side-utility utility-function k-step 0 "yes"
;  let no-side-utility utility-function 0 k-step "no"

  report one-of list (list k-step 0)　(list 0 (- k-step))
end
...
to-report market-prediction-mean
  ifelse length yes-price-record >= 1 [report mean yes-price-record] [report "nan"]
end
