...
to go
  repeat market-attempts [
    ...
    record
    if (max list weighted-prediction (1 - weighted-prediction)) > (1 - 1e-4) [stop]
  ]
end
...
to-report decision-function ; in bettor ; box
  let gap (q-yes - q-no)
  let k risk-neutral-optimality gap
  ;let k CARA-adjusted-optimality gap
...
to-report CARA-loss-aversion-utility [k]
  let eps 1e-8
  let k_ ifelse-value (abs k > constraint) [ ifelse-value (k > 0) [ constraint ] [ (- constraint) ]] [ k ]
  let dc (liquidity * ln (exp ((q-yes + k_) / liquidity) + exp (q-no / liquidity) + eps)) - (liquidity * ln (exp (q-yes / liquidity) + exp (q-no / liquidity) + eps))
  let wY k_ - dc
  let wN (- dc)
  if abs CARA < eps [
...
to-report CARA-loss-aversion-optimality [g]
  ...
  let k-neg ((g - liquidity * ln ((1 - effective-belief) / (loss-aversion * effective-belief))) / denom)
...
to normalize-score ; box
  ...
  let baseline mean [score] of bettors
  let denom standard-deviation [score] of bettors
