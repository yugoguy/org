...
to initailize-globals ; box
  set q-yes 0
  set q-no  0
  set price-yes 0.5
  set price-no 0.5
  set-default-shape bettors "person"
  set intervened false
end
...
to make-bet
  let decision optimal-bet-position
  resolve-position decision
end
...
to resolve-position [decision] ; in bettor ; box
  ...
  set size max list 0.5 (log (abs (pos-yes - pos-no)) 10)
...
to normalize-score ; box
  let norm sum [score] of bettors
  ask bettors [
    set score score / norm
  ]
end
...
to reset
  reset-bettors-belief
  reset-market-states
  reset-ticks
end

to reset-bettors-belief ; box
  ask bettors [
    set color gray
    set pos-yes 0
    set pos-no  0
